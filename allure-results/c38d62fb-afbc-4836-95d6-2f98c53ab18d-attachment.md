...
```ts
  ...
  8  | 
  9  |     //log in details
> 10 |   await LoginPage.username.fill('standard_user');
     |                            ^ TypeError: Cannot read properties of undefined (reading 'fill')
  11 |   await LoginPage.Password.fill('secret_sauce');
  12 | 
  13 |     //click on Main page
  14 |   /*const mainpage = new MainPage(page);
  15 |   await mainpage.ClickOnItems();
  16 | 
  17 |   //Click on Home page link
  18 |   const Cartpage = new CartPage(page);
  19 |   await CartPage.ClickOnAddToCart();*/
