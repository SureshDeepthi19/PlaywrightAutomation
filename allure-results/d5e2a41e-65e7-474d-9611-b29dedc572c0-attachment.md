...
- Location: tests\Sauce_pom_demo.spec.js:6:1

# Error details

```
TypeError: mainpage.clickOnAllItemsMenu is not a function
```
...
```ts
  ...
  5  | 
  6  | test("Navigating to home page  webiste using Playwright", async({page})=>{
  ...
  12 | 
  13 |     //click on Main page
  14 |   const mainpage = new MainPage(page);
  15 |   await mainpage.clickOnBurgerButton();
> 16 |   await mainpage.clickOnAllItemsMenu();
     |                  ^ TypeError: mainpage.clickOnAllItemsMenu is not a function
  17 | 
  18 |   /*//Click on Home page link
  19 |   const Cartpage = new CartPage(page);
  20 |   await CartPage.ClickOnAddToCart();*/
